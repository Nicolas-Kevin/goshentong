...
func (stmt *Stmt) query(binds []bindStruct) (driver.Rows, error) {
	...
	if stmt.conn.prefetchMemory > 0 {
		...
		err = stmt.conn.ociAttrSet(unsafe.Pointer(stmt.stmt), C.OCI_HTYPE_STMT, unsafe.Pointer(&prefetchMemory), 0, C.OCI_ATTR_PREFETCH_ROWS)
		if err != nil {
			return nil, err
		}
	}
...
func (stmt *Stmt) getRowid() (string, error) {
	rowidP, _, err := stmt.conn.ociDescriptorAlloc(C.OCI_DTYPE_LOB, 0)
	if err != nil {
		return "", err
	}
